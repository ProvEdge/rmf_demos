...
import {
  ...
  NotificationTypes,
} from "../fixed-components/notification-snackbar";
...
const LoopRequestForm = (props: LoopFormProps): React.ReactElement => {
    ...
    try {
      ...
      if (isFormValid()) {
        ...
        if (response && response["task_id"] === "") {
          ...
          showSuccessSnackbar(
            `Request submitted successfully! Task ID: [${response["task_id"]}]`
          );
        }
        cleanUpForm();
      }
  ...
  return (
    <Box className={classes.form} role="loop-request-form">
      ...
      <div className={classes.divForm}>
        <div className="mb-4">
          <label className="form-label fw-bold">Select end location</label>
      ...
      <div className={classes.divForm}>
        <label className="form-label fw-bold">Number of Loops</label>
        <input
          type="number"
      ...
      <div className={classes.buttonContainer}>
        ...
        {openSnackbar && (
          <NotificationSnackbar
            ...
            closeSnackbarCallback={() => setOpenSnackbar(false)}
          />
        )}
      </div>
    </Box>
